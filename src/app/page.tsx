...
export default function HomePage() {
  return (
    <>
      ...
      <div className="mx-auto max-w-3xl px-4 pt-6 sm:pt-10 pb-4">
        <p className="text-center text-sm font-semibold text-sun-700 uppercase tracking-wider">
          Reapply on time. Every time.
        </p>
        <h1 className="mt-2 text-center text-3xl sm:text-4xl font-extrabold tracking-tight text-ink">
          Smart sunscreen timer
        </h1>
        <p className="mt-3 text-center text-base sm:text-lg text-ink-soft max-w-xl mx-auto">
      ...
      <section className="mx-auto max-w-3xl px-4 mt-12 prose-article">
        ...
        <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-2 !list-none !pl-0 mt-2 not-prose">
          {subpages.map((p) => (
            <li key={p.slug}>
              <Link
                href={`/${p.slug}`}
      ...
      <section id="faq" className="mx-auto max-w-3xl px-4 mt-12 prose-article">
        ...
        {faq.map((item) => (
          <details
            key={item.q}
            className="group border-b border-sun-100/70 py-4"
          >
